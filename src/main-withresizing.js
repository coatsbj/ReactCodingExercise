...
var ChartApplication = React.createClass({
	updateStockHistory: function (optionsData) { 
		...
		($ || jQuery).get('api/stock-history/' + stockSymbolsString + '/' + daysHistory, function(data, status, jqXhr) {
			...
			Object.defineProperties(persistableOptions, {
				...
				windowHeight: {enumerable: true, value: this.state.windowHeight || window.innerHeight},
				canvasHeight: {enumerable: true, value: this.state.canvasHeight || (window.innerHeight - 150)}
			});
	...
	handleResize: function (e) { 
		...
		var lineChart = this.refs['line-graph'];
		var canvas = lineChart && lineChart.getCanvas();
		if (!canvas) { this.setState({	windowHeight: winH }); return; }
		
		var jq = ($ || jQuery);
		var canvTop = jq ? jq(canvas).position().top : canvas.offsetTop;
		var canvH = jq ? jq(canvas).height() : canvas.height;
		var canvBtm = canvTop + canvH;

		if (canvBtm > (winH - 10)) {
			canvH =  winH - (canvTop + 10);
			if (canvH < 200) { this.setState({ windowHeight: winH }); return; }
		}
		console.log('Old canvas height: ' + this.state.canvasHeight);
		console.log('New canvas height: ' + canvH);
		this.setState({ windowHeight: winH, canvasHeight: canvH});
	},
	...
	getInitialState: function () { 
		...
		return { 
			...
			windowHeight: this.props.initialWindowHeight,
			canvasHeight: this.props.initialWindowHeight - 150
		};
	},
	render: function () { 
		var chartOptions = {
		...
		var chartStyles = { maxHeight: this.state.chartHeight };
		return (
...
ReactDom.render(<ChartApplication initialWindowHeight={window.innerHeight} />, ($ || jQuery)('.react-app')[0]);